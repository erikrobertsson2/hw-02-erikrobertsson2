...
edibnb_medians <- edibnb_ordered_by_med |>
  group_by(neighbourhood) |>
  summarize(median = median(review_scores_rating)) |>
  arrange(desc(median))

edibnb_ordered_by_med <- edibnb_ordered_by_med |>
  inner_join(edibnb_medians, by = c("neighbourhood"))

edibnb_ordered_by_med$neighbourhood <- fct_reorder(edibnb_ordered_by_med$neighbourhood, edibnb_medians$median)

ggplot(edibnb_ordered_by_med, aes(x = review_scores_rating, y = neighbourhood, fill = neighbourhood)) +
  geom_density_ridges(alpha = 0.5, scale = 1) +
  scale_color_brewer(palette = "Set3") +
  theme_ridges() +
  labs(x = "Review Score", y = "Neighbourhood", color = "Neighbourhood") +
...
From the plot, we can see that the distributions of review scores vary significantly across neighborhoods. Some neighborhoods have a large number of reviews with scores around 100, such as the Morningside neighbourhood, indicating a high overall level of satisfaction among guests. On the other hand, some neighborhoods have a wider distribution of scores, such as the Old Town neighbourhood, indicating a wider range of opinions among guests.
